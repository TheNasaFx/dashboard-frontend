              ...
              <div class="card">
                ...
                <div class="card-body pt-0">
                  ...
                  <div v-if="step === 2" class="setup-step">
                    ...
                    <!-- Loading state -->
                    <div v-if="loading && !qrCodeData" class="text-center mb-4">
                      <div class="spinner-border text-primary" role="status">
                        <span class="visually-hidden">Loading...</span>
                      </div>
                      <p class="text-muted mt-2">QR код үүсгэж байна...</p>
                    </div>
                    
                    <div v-if="qrCodeData" class="text-center mb-4">
                    ...
                    <div v-if="qrCodeData" class="manual-entry-section">
                    ...
                    <div v-if="qrCodeData" class="d-grid mt-4">
                      <button
                        @click="step = 3"
...
const step = ref(2) // Start from step 2 (QR code)
const username = ref('')
const password = ref('')
...
// Check if user came from login page with credentials
onMounted(async () => {
  const tempUsername = sessionStorage.getItem('temp_username')
  const tempPassword = sessionStorage.getItem('temp_password')
  const isReset = sessionStorage.getItem('reset_2fa')
  
  if (tempUsername && tempPassword) {
    username.value = tempUsername
    password.value = tempPassword
    
    // If this is a reset, first disable existing 2FA
    if (isReset === 'true') {
      try {
        loading.value = true
        
        const resetResponse = await fetch('http://localhost:8080/api/v1/auth/reset-2fa', {
          method: 'POST',
          headers: {
        ...
        if (!resetResponse.ok || !resetData.success) {
          console.warn('2FA reset failed, proceeding anyway:', resetData.error)
        }
        
        // Clear reset flag
        sessionStorage.removeItem('reset_2fa')
      } catch (err) {
        console.warn('2FA reset error, proceeding anyway:', err)
      } finally {
        loading.value = false
      }
    }
    
    // Auto-proceed to QR generation
    generateQRCode()
  } else {
    // If no credentials in session, redirect to login
    router.push('/login')
  }
})
...
async function verifySetup() {
  ...
  try {
    ...
    success.value = '2FA амжилттай идэвхжлээ! Нэвтэрж байна...'
    
    // Now login with credentials since 2FA is enabled
    ...
    if (!loginResponse.ok || !loginData.success) {
      throw new Error(loginData.error || 'Нэвтрэх алдаа')
    }
    ...
    const payload = parseJwt(loginData.data.token)
    setUser({
      name: loginData.data.fullName || '',
      avatar: '',
      logged: true,
